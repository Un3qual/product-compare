defmodule Mix.Tasks.ProductCompare.Ingestion.CjApplicationCohortStartupTest do
  ...
  test "report-only cohort task does not start CJ scheduler children" do
    ...
    assert_raise Mix.Error, "invalid review status: approved", fn ->
      capture_io(fn -> CjApplicationCohort.run(["--status", "approved"]) end)
    end
