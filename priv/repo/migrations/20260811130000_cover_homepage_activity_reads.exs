defmodule ProductCompare.Repo.Migrations.CoverHomepageActivityReads do
  use Ecto.Migration
  ...
  def up do
    ensure_concurrent_index(
      @new_index,
      " USING btree (inserted_at, merchant_product_id) " <>
        "INCLUDE (user_id, anonymous_visitor_id)",
      "CREATE INDEX CONCURRENTLY #{quote_identifier(@new_index)} " <>
        "ON #{qualified_name(:commerce_click_sessions)} (inserted_at, merchant_product_id) " <>
        "INCLUDE (user_id, anonymous_visitor_id)"
    )
  ...
  def down do
    ensure_concurrent_index(
      ...
      "CREATE INDEX CONCURRENTLY #{quote_identifier(@old_index)} " <>
        "ON #{qualified_name(:commerce_click_sessions)} (inserted_at)"
    )
  ...
  defp index_state(index, expected_suffix) do
    case repo().query!(
      ...
      [[true, false, "commerce_click_sessions", definition]] ->
