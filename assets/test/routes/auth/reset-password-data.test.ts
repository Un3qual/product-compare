...
test("owns the exact missing-token error and preserves its identity", () => {
  ...
  expect(resetPasswordErrorsForToken("")).toEqual([RESET_PASSWORD_MISSING_TOKEN_ERROR]);
  expect(resetPasswordErrorsForToken("")[0]).toBe(RESET_PASSWORD_MISSING_TOKEN_ERROR);
  expect(resetPasswordErrorsForToken("reset-token")).toEqual([]);
});

test("owns the exact reset-password success copy", () => {
  expect(CREDENTIAL_RESET_COMPLETION_MESSAGE).toBe("Your password has been updated.");
});
