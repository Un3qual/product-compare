...
describe("getRouteErrorViewData", () => {
  test("capitalizes the default unavailable resource copy", () => {
    expect(getRouteErrorViewData({ resourceName: "revenue report", error: { kind: "unknown" } }))
      .toEqual({
  ...
  test.each([
    [503, "A server error occurred while loading the comparison.", "Please try refreshing the page or come back later."],
    [404, "The requested comparison could not be found.", "Please check the URL and try again."],
    [401, "You don't have permission to view this comparison.", "Please sign in or contact support if you believe this is an error."],
    [403, "You don't have permission to view this comparison.", "Please sign in or contact support if you believe this is an error."],
    [422, "An error occurred while loading the comparison.", "Please try refreshing the page."]
  ] as const)("returns exact response copy for status %s", (status, errorMessage, retryGuidance) => {
    expect(getRouteErrorViewData({ resourceName: "comparison", error: { kind: "response", status } }))
  ...
  test.each([
    ...
    expect(getRouteErrorViewData({ resourceName: "merchant directory", error: { kind: "error", error } }))
      .toEqual({
