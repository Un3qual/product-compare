    ...
    resolveCommunityContentRemovalMessage(
      {
        ...
        errors: [
          { code: "INVALID_LIFECYCLE", message: "Removed community content cannot be changed." },
        ],
      },
      [],
    ),
...
test.each([
  ...
  (_kind, completionKey, resolveMessage) => {
    ...
    expect(resolveMessage({ [completionKey]: null, errors: [mutationError] }, [])).toBe(
      "Authored text is invalid.",
    );
    expect(resolveMessage({ [completionKey]: null, errors: [mutationError] }, [graphQLError])).toBe(
      "Request failed. Please try again.",
    );
  },
);
