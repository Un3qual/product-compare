...
import { CategoryRoute } from "../../../src/routes/categories/CategoryRoute";
import { categoryLoader } from "../../../src/routes/categories/loader";
...
test("category loader returns 404 for invalid slugs and canonical metadata for curated categories", async () => {
  ...
  const invalid = await categoryLoader({ context: createRelayRouterContext(environment), params: { slug: "Bad Slug" }, request: new Request("https://app.example/categories/Bad%20Slug") } as never);
  expect((invalid as { init: { status: number } }).init.status).toBe(404);

  mockedFetchRouteQuery.mockResolvedValueOnce({
    data: { category: { seo: { title: "Compare Cameras", description: "Trusted camera evidence", canonicalPath: "/categories/cameras", indexable: true, imageUrl: null, structuredData: null } } },
    descriptor: { descriptor: true },
    dispose: vi.fn()
  } as never);
  const result = await categoryLoader({ context: createRelayRouterContext(environment), params: { slug: "cameras" }, request: new Request("https://app.example/categories/cameras") } as never);
  expect(result).toMatchObject({ status: "ready", metadata: { canonicalUrl: "https://app.example/categories/cameras", indexable: true } });
});
