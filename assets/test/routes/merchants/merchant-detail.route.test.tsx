...
} from "../../../src/routes/merchants/detail/MerchantDetailRoute";
...
test("merchant detail loader returns HTTP 404 for malformed and unknown slugs", async () => {
  ...
  const invalid = await merchantDetailLoader({
    ...
    request: new Request("https://example.test/merchants/Bad%20Slug"),
  } as never);
  ...
  const unknown = await merchantDetailLoader({
    context: createRelayRouterContext(environment),
    params: { slug: "missing-12345678" },
    request: new Request("https://example.test/merchants/missing-12345678"),
  } as never);
...
test("MerchantDetailRoute renders complete summary, safe destination, and product offer evidence", () => {
  ...
  expect(screen.getByLabelText("Merchant coverage")).toHaveTextContent(
    "Offers with complete prices1",
  );
  expect(screen.getByText(/Current products and offer details/)).toBeVisible();
  expect(
    screen.queryByText(/offer evidence|observation|stale|unobserved/i),
