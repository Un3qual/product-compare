...
| ready | Database constraint application parity | The repository-wide catalog audit found reachable same-row checks with missing pre-write validation or Ecto error mappings, plus two discussion read-modify-write boundaries that did not enforce transaction ownership. | Execute the approved plan as one batch. Keep PostgreSQL authoritative, remove only two redundant price lower-bound checks, retain the slug-reservation trigger exception, add concise repository guidance, and do not add a registry, DSL, source scanner, or catalog-driven CI policy. |
