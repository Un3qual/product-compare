...
| `affiliate_networks.*` | `commerce_links.network?` enum + config | Network metadata moves to application config or a lightweight `commerce_network_configs` reference table; `commerce_links.network` and `commerce_conversions.source_network` use the same enum, with `commerce_links.network` null for `non_affiliate` rows and also temporarily nullable for Phase 1 backfilled `affiliate` rows whose legacy `affiliate_network_id` is absent or not yet mappable. |
