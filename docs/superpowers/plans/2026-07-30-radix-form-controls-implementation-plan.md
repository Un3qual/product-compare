...
Rebase `Button` and `TextField` on Radix Themes, add equally small TextArea,
Checkbox, and Radio wrappers, and use the Radix Select primitive behind the
project Select wrapper so empty filter values retain exact form semantics.
Then migrate visible raw controls route by route. Hidden GET/pagination fields
remain native and visually inert.
...
- [x] Inventory every visible raw form control and every use of the project
  `Button` and `TextField` wrappers.
- [x] Add focused accessibility and form-submission characterization for
  selects, text areas, checkboxes, date inputs, and `asChild` button links.

## Task 2: Establish The Radix Themes Boundary

- [x] Add the exact Radix Themes dependency and stylesheet.
- [x] Install one provider at the shared application boundary with SSR-safe
  output.
- [x] Rebase `Button` and `TextField` and add only the Select, TextArea,
  Checkbox, and Radio wrappers required by live controls.
- [x] Preserve project data attributes and semantic variants only where they
  remain useful to application-owned StyleX/CSS.

## Task 3: Migrate Visible Controls

- [x] Replace visible raw text inputs, selects, text areas, and checkboxes in
  affiliate, catalog, comparison, ingestion, merchant, offer, and
  community routes.
- [x] Leave hidden transport fields native and document that exception in the
  boundary test.
- [x] Prove no visible raw control remains where a project Radix wrapper
  exists.

## Task 4: Verify And Commit

- [x] Run focused UI/route tests, accessibility assertions, TypeScript, Oxc,
  the full frontend suite, client and SSR builds, and the bundle contract.
- [x] Record exact results in `docs/work/frontend-radix-form-controls.md`.
- [x] Commit with `refactor: adopt radix form controls`.
