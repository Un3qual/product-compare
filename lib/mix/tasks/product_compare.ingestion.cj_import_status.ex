defmodule Mix.Tasks.ProductCompare.Ingestion.CjImportStatus do
  @moduledoc "Reports persisted CJ product import status without contacting CJ."
  ...
  defp enforce_required_success(%{require_success: true, latest_success: nil}) do
    Mix.raise("no successful CJ product import found")
  end
  ...
  defp enforce_required_success(%{
    ...
    Mix.raise("latest successful CJ product import is stale")
  end

  defp enforce_required_success(report), do: report

  defp render_report(%{latest_run: latest_run, latest_success: latest_success, fresh: fresh}) do
    [
