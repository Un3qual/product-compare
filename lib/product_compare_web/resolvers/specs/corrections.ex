defmodule ProductCompareWeb.Resolvers.Specs.Corrections do
  @moduledoc false
  ...
  @spec propose(any(), %{input: map()}, Absinthe.Resolution.t()) :: {:ok, map()}
  def propose(_parent, %{input: input}, %{context: %{current_user: user}}) do
  ...
  def propose(_parent, _args, _resolution) do
    {:ok, correction_error_payload(GraphQLErrors.unauthenticated_mutation_error())}
  end

  @spec moderate(any(), %{input: map()}, Absinthe.Resolution.t()) :: {:ok, map()}
  def moderate(_parent, %{input: input}, resolution) do
  ...
  @spec value_text(SpecificationCorrection.t(), map(), Absinthe.Resolution.t()) ::
          {:ok, String.t()}
  def value_text(correction, _args, _resolution) do
  ...
  defp correction_error_payload(error), do: %{correction: nil, errors: [error]}
